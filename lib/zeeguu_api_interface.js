...
var multi_dicts = [babla_da_data, babla_no_data, wordreference_data, sensagent_data, dict_cc];
...
//    console.log(API_URL + parameters.join("/") + "?session=" + state.session);
...
function get_translation_from_the_server(word_to_lookup, url, context, title, func_that_uses_translation) {
    console.log(word_to_lookup);

    $.post(
        api_url(["translate_and_bookmark",
                state.from,
                state.base_language]),
            {
                "context": context,
